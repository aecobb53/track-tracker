...
from .common import ATHLETE_FILTER_PARAMS, ATHLETE_ARRANGE_PARAMS, ATHLETE_DISPLAY_PARAMS, DEV_ROADMAP, display_date, class_formatter

async def tech_stack_html_page():
...
async def development_roadmap_html_page():
    base_doc = await project_dev_base_page()
    page_content = Div().add_class('page-content')
    ...
    """
    Create a local file for the current roadmap and whats been completed and what hasnt
    """
...
async def service_info_html_page():
    base_doc = await project_dev_base_page()
    page_content = Div().add_class('page-content')
    ...
    """
    Hit the service and get back info i guess...
    Maybe info about what is being sent to the service like phone, computer 
