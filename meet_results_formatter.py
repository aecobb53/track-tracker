...
def parse_data_row(data_row: list[str], event: str, header: list[str], meet_name: str = None):
    ...
    if len(header) == 7 and len(data_row) == 4:
        if header == ['PLACE', 'VIDEO', 'ATHLETE', 'TEAM', 'MARK', 'WIND', 'HEAT']:
            ...
            result_wind_re = re.search(r'[\s\t]+((\d+:)?\d+\.?\d*)[\s\t]+([-+])[\s\t]*(\d+.?\d*)[\s\t]+(\d+)', data_row[3])
            result_no_wind_re = re.search(r'[\s\t]+((\d+:)?\d+?\.\d+)[\s\t]+(\d+)', data_row[3])
            result_high_jump_re = re.search(r'[\s\t]+(\d+)-(\d+\.?\d*)[\s\t]+(\d+)', data_row[3])
            result_long_jump_re = re.search(r'[\s\t]+(\d+)-(\d+\.?\d*)[\s\t]+([-+])(\d*\.?\d*)[\s\t]+(\d+)', data_row[3])
            if result_wind_re:
                mark, _, wind_sign, wind, heat = result_wind_re.groups()
                team = data_row[3][:result_wind_re.start(0)].strip()
            elif result_no_wind_re:
                mark, _, heat = result_no_wind_re.groups()
                team = data_row[3][:result_no_wind_re.start(0)].strip()
            elif result_high_jump_re:
                attempt, mark, heat = result_high_jump_re.groups()
                team = data_row[3][:result_high_jump_re.start(0)].strip()
            elif result_long_jump_re:
                attempt, mark, wind_sign, wind, heat = result_long_jump_re.groups()
                team = data_row[3][:result_long_jump_re.start(0)].strip()
            else:
                x=1
            ...
            return data_obj
    elif len(header) == 5 and len(data_row) == 4:
        if header == ['PLACE', 'VIDEO', 'TEAM', 'MARK', 'HEAT']:
            mark = None
            heat = None

            result_relay_re = re.search(r'((\d+:)?\d+\.?\d*)[\s\t]+(\d+)', data_row[3])
            if result_relay_re:
...
def parse_meet_file(path: str):
    ...
    if meet_name in ['ATB Churchyard PV Meet', 'Arcadia Invitational']:
        x=1
    ...
    for index, line in enumerate(data):
        if meet_name == 'Colorado State Track and Field Championships' and index == 14976:
            x=1
        newline_re = re.search(r'^\d+$', line.strip())
        if newline_re:
            row = parse_data_row(data_row=data_row, event=event, header=header, meet_name=meet_name)
            if row:
        ...
        if any([re.search(i, line.replace("'", '').strip().lower()) for i in re_strings]):
            if data_row:
                data_row.pop(-1)

            row = parse_data_row(data_row=data_row, event=event, header=header)
            if row:
                meet_data.append(row)
            data_row = []
            event = line
    ...
    if meet_name in ['Fairview JV Invite', 'ATB Churchyard PV Meet', 'Arcadia Invitational']:
        x=1
